...
expName = 'dynamind-01'  # from the Builder filename that created this script
expVersion = '1.0.0'
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\Maik Bieleke\\My Drive\\Labor\\dfg-dynamind\\dynamind-01\\dynamind-01.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    NUM_MVC               = 2     # How many MVC trials to conduct
    ...
    FORCE_TOLERANCE_BUFFER = deque(maxlen = FORCE_BUFFER_LENGTH)
    
    def force_nrmse(force_samples, target_force):
        """
        Normalized RMSE of force relative to target.
        Returns a percentage (e.g., 10% deviation from target).
        """
        force = np.asarray(force_samples)
        rmse  = np.sqrt(np.mean((force - target_force) ** 2))
        return round((rmse / target_force) * 100, 2)
    
    def force_bias(force_samples, target_force):
        """
        Mean signed deviation as a percentage of target.
        Positive = pressed too hard on average, negative = too soft.
        """
        force = np.asarray(force_samples)
        return round((np.mean(force - target_force) / target_force) * 100, 2)
    
    def force_pct_in_range(force_samples, target_force, tolerance=TOLERANCE):
        """
        Percentage of samples within ±tolerance Newtons of target.
        """
        force = np.asarray(force_samples)
        lower = target_force - tolerance
        upper = target_force + tolerance
        return round(np.mean((force >= lower) & (force <= upper)) * 100, 2)
    ...
    BLOCK_BREAK_DURATION = 4 * 60 # four minutes break per default
    ...
    # SPEEDRUN MODE ----
    speedrun = expInfo['speedrun']
    if speedrun:
        NUM_BLOCKS            = 1
        NUM_LEVELS            = 2
        NUM_PALETTES_EXERCISE = 5
        NUM_PALETTES_REGULAR  = 5
        NUM_TARGETS           = 1
    ...
    # RANDOMNESS ----
    def seed_from_participant_id(pid):
        pid_text = str(pid).strip()
        if pid_text == "":
            pid_text = "EMPTY_PARTICIPANT_ID"
        digest = hashlib.sha256(pid_text.encode("utf-8")).digest()
        # use first 8 bytes, fold to uint32 for NumPy compatibility
        return int.from_bytes(digest[:8], byteorder="big", signed=False) % (2**32)
    
    # Global seed 
    GLOBAL_SEED = seed_from_participant_id(expInfo['participant'])
    
    # Seed Python stdlib RNG (used by random.choice/sample/random/randint)
    random.seed(GLOBAL_SEED)
    
    # Seed NumPy legacy RNG (used by np.random and imported randint/shuffle/etc.)
    ...
    # Save seed
    for row in rows:
        row["seed"] = GLOBAL_SEED
    
    pd.DataFrame(rows).to_csv(thisExp.dataFileName + '_data.csv', index=False)
    ...
    # --- Initialize components for Routine "rr_block" ---
    ...
    # --- Initialize components for Routine "rr_start_block" ---
    start_block_text = visual.TextStim(win=win, name='start_block_text',
        ...
        languageStyle='LTR',
        depth=-1.0);
    start_block_button = visual.ButtonStim(win, 
        text='', font='Courier New',
        pos=(0, -0.2),
        letterHeight=0.025,
        size=(0.4, 0.1), 
        ...
        padding=None,
        anchor='center',
        name='start_block_button',
        depth=-2
    )
    start_block_button.buttonClock = core.Clock()
    ...
    # --- Initialize components for Routine "rr_finish" ---
    finish_display = visual.TextStim(win=win, name='finish_display',
        text='Das Experiment ist beendet.\n\nDieses Fenster schließt sich in wenigen Sekunden.',
        font='Arial',
        pos=(0, 0), draggable=False, height=0.05, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
    ...
    for thisOo_block in oo_block:
        ...
        # --- Prepare to start Routine "rr_block" ---
        # create an object to store info about Routine rr_block
        rr_block = data.Routine(
            name='rr_block',
            components=[],
        )
        rr_block.status = NOT_STARTED
        continueRoutine = True
        # update component parameters for each repeat
        # Run 'Begin Routine' code from block_code
        is_mvc = int(oo_block.thisN == 0 or oo_block.thisN == len(blocks))
        is_block = int(oo_block.thisN < len(blocks))
        mvc_phase = 'mvc_base' if oo_block.thisN == 0 else 'mvc_post'
        
        if oo_block.thisN == 0:
            maxforce_type = ["Practice", "Calibration"]
            maxforce_repetitions = [999, NUM_MVC]
            maxforce_duration = [999, 45]
        else:
            maxforce_type = ["Calibration"]
            maxforce_repetitions = [NUM_MVC]
            maxforce_duration = [45]
        # store start times for rr_block
        rr_block.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
        rr_block.tStart = globalClock.getTime(format='float')
        rr_block.status = STARTED
        thisExp.addData('rr_block.started', rr_block.tStart)
        rr_block.maxDuration = None
        # keep track of which components have finished
        rr_blockComponents = rr_block.components
        for thisComponent in rr_block.components:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        # --- Run Routine "rr_block" ---
        thisExp.currentRoutine = rr_block
        rr_block.forceEnded = routineForceEnded = not continueRoutine
        while continueRoutine:
            ...
            # has a Component requested the Routine to end?
            if not continueRoutine:
                rr_block.forceEnded = routineForceEnded = True
            # has the Routine been forcibly ended?
            if rr_block.forceEnded or routineForceEnded:
                break
            # has every Component finished?
            continueRoutine = False
            for thisComponent in rr_block.components:
        ...
        # --- Ending Routine "rr_block" ---
        for thisComponent in rr_block.components:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        # store stop times for rr_block
        rr_block.tStop = globalClock.getTime(format='float')
        rr_block.tStopRefresh = tThisFlipGlobal
        thisExp.addData('rr_block.stopped', rr_block.tStop)
        # the Routine "rr_block" was not non-slip safe, so reset the non-slip timer
        routineTimer.reset()
        
        # set up handler to look after randomisation of conditions etc
        yn_mvc = data.TrialHandler2(
            name='yn_mvc',
            nReps=is_mvc, 
            method='random', 
            extraInfo=expInfo, 
        ...
        thisExp.addLoop(yn_mvc)  # add the loop to the experiment
        thisYn_mvc = yn_mvc.trialList[0]  # so we can initialise stimuli with some values
        # abbreviate parameter names if possible (e.g. rgb = thisYn_mvc.rgb)
        if thisYn_mvc != None:
            for paramName in thisYn_mvc:
                globals()[paramName] = thisYn_mvc[paramName]
        if thisSession is not None:
            # if running in a Session with a Liaison client, send data up to now
            thisSession.sendExperimentData()
        
        for thisYn_mvc in yn_mvc:
            yn_mvc.status = STARTED
            if hasattr(thisYn_mvc, 'status'):
                thisYn_mvc.status = STARTED
            currentLoop = yn_mvc
            thisExp.timestampOnFlip(win, 'thisRow.t', format=globalClock.format)
            if thisSession is not None:
                # if running in a Session with a Liaison client, send data up to now
            ...
            continueRoutine = True
            # update component parameters for each repeat
            # Run 'Begin Routine' code from start_mvc_code
            ...
            while continueRoutine:
                # if trial has changed, end Routine now
                if hasattr(thisYn_mvc, 'status') and thisYn_mvc.status == STOPPING:
                    continueRoutine = False
                # get current time
                t = routineTimer.getTime()
                tThisFlip = win.getFutureFlipTime(clock=routineTimer)
            ...
            yn_mvc.addData('start_mvc_button.numClicks', start_mvc_button.numClicks)
            if start_mvc_button.numClicks:
               yn_mvc.addData('start_mvc_button.timesOn', start_mvc_button.timesOn)
               yn_mvc.addData('start_mvc_button.timesOff', start_mvc_button.timesOff)
            else:
               yn_mvc.addData('start_mvc_button.timesOn', "")
               yn_mvc.addData('start_mvc_button.timesOff', "")
            ...
            for thisOo_mvc_type in oo_mvc_type:
                ...
                oo_mvc_rep = data.TrialHandler2(
                    name='oo_mvc_rep',
                    nReps=maxforce_repetitions[oo_mvc_type.thisN], 
                    method='sequential', 
                    extraInfo=expInfo, 
                    originPath=-1, 
                    trialList=[None], 
                ...
                for thisOo_mvc_rep in oo_mvc_rep:
                    ...
                    countdown_lights_led_off_sent = False
                        ...
                        if countdown_lights.status == STARTED:
                            ...
                            if tThisFlipGlobal > countdown_lights.tStartRefresh + 4-frameTolerance:
                                ...
                                if False:
                                   ...
                                   countdown_lights_led_off_sent = True
                    ...
                    if False and not countdown_lights_led_off_sent:
                        countdown_lights.turnOffLights("inner")
                    ...
                    # Get current state
                    cur_maxforce_type        = maxforce_type[oo_mvc_type.thisN]
                    cur_maxforce_repetitions = maxforce_repetitions[oo_mvc_type.thisN]
                    cur_maxforce_duration    = maxforce_duration[oo_mvc_type.thisN]
                    maxforce_lights_led_off_sent = False
                    ...
                    while continueRoutine and routineTimer.getTime() < 3.5:
                        ...
                        if cur_maxforce_type == "Calibration":
                            mvc = max(mvc, cur_mvc)
                        
                        # Display text
                        maxforce_display_text = (
                            f"============ TRIAL INFO ============\n"
                            f"Trial Type:        {cur_maxforce_type} ({oo_mvc_rep.thisN+1}/{cur_maxforce_repetitions})\n"
                        ...
                        if maxforce_lights.status == STARTED:
                            ...
                            if tThisFlipGlobal > maxforce_lights.tStartRefresh + 3.0-frameTolerance:
                                ...
                                if True:
                                   ...
                                   maxforce_lights_led_off_sent = True
                    ...
                    if True and not maxforce_lights_led_off_sent:
                        maxforce_lights.turnOffLights("all")
                    ...
                    while continueRoutine:
                        ...
                        # Measure time
                        remaining = max(0, cur_maxforce_duration - t)
                        if remaining <= 0:
                            continueRoutine = False
                        ...
                        if mvc_stop_practice.status == NOT_STARTED and cur_maxforce_type == "Practice":
                            # keep track of start time/frame for later
                ...
                # completed maxforce_repetitions[oo_mvc_type.thisN] repeats of 'oo_mvc_rep'
                ...
                if oo_mvc_rep.trialList in ([], [None], None):
                    params = []
                else:
                    params = oo_mvc_rep.trialList[0].keys()
                # save data for this loop
                oo_mvc_rep.saveAsExcel(filename + '.xlsx', sheetName='oo_mvc_rep',
                    stimOut=params,
            ...
            # completed len(maxforce_type) repeats of 'oo_mvc_type'
            ...
            if thisSession is not None:
                ...
                thisSession.sendExperimentData()
            # get names of stimulus parameters
            if oo_mvc_type.trialList in ([], [None], None):
                params = []
            else:
                params = oo_mvc_type.trialList[0].keys()
            # save data for this loop
            oo_mvc_type.saveAsExcel(filename + '.xlsx', sheetName='oo_mvc_type',
                stimOut=params,
                dataOut=['n','all_mean','all_std', 'all_raw'])
            # mark thisYn_mvc as finished
            if hasattr(thisYn_mvc, 'status'):
                thisYn_mvc.status = FINISHED
            # if awaiting a pause, pause now
            if yn_mvc.status == PAUSED:
                thisExp.status = PAUSED
                pauseExperiment(
                    thisExp=thisExp, 
                    win=win, 
        ...
        # completed is_mvc repeats of 'yn_mvc'
        yn_mvc.status = FINISHED
        
        if thisSession is not None:
            # if running in a Session with a Liaison client, send data up to now
            thisSession.sendExperimentData()
        # get names of stimulus parameters
        if yn_mvc.trialList in ([], [None], None):
            params = []
        else:
            params = yn_mvc.trialList[0].keys()
        # save data for this loop
        yn_mvc.saveAsExcel(filename + '.xlsx', sheetName='yn_mvc',
            stimOut=params,
            dataOut=['n','all_mean','all_std', 'all_raw'])
        
        # set up handler to look after randomisation of conditions etc
        yn_block = data.TrialHandler2(
            name='yn_block',
            nReps=is_block, 
            method='sequential', 
            extraInfo=expInfo, 
        ...
        thisExp.addLoop(yn_block)  # add the loop to the experiment
        thisYn_block = yn_block.trialList[0]  # so we can initialise stimuli with some values
        # abbreviate parameter names if possible (e.g. rgb = thisYn_block.rgb)
        if thisYn_block != None:
            for paramName in thisYn_block:
                globals()[paramName] = thisYn_block[paramName]
        if thisSession is not None:
            # if running in a Session with a Liaison client, send data up to now
            thisSession.sendExperimentData()
        
        for thisYn_block in yn_block:
            yn_block.status = STARTED
            if hasattr(thisYn_block, 'status'):
                thisYn_block.status = STARTED
            currentLoop = yn_block
            thisExp.timestampOnFlip(win, 'thisRow.t', format=globalClock.format)
            if thisSession is not None:
                # if running in a Session with a Liaison client, send data up to now
                thisSession.sendExperimentData()
            ...
            # --- Prepare to start Routine "rr_start_block" ---
            # create an object to store info about Routine rr_start_block
            rr_start_block = data.Routine(
                name='rr_start_block',
                components=[start_block_text, start_block_button],
            )
            rr_start_block.status = NOT_STARTED
            continueRoutine = True
            # update component parameters for each repeat
            # Run 'Begin Routine' code from start_block_code
            # Define button for block start
            cur_phase = blocks[oo_block.thisN][0].get('phase', 'main')
            start_block_button_text = f"{phase_labels.get(cur_phase, cur_phase)}"
            
            start_block_text.setText(start_block_button_text)
            # reset start_block_button to account for continued clicks & clear times on/off
            start_block_button.reset()
            # store start times for rr_start_block
            rr_start_block.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
            rr_start_block.tStart = globalClock.getTime(format='float')
            rr_start_block.status = STARTED
            thisExp.addData('rr_start_block.started', rr_start_block.tStart)
            rr_start_block.maxDuration = None
            # keep track of which components have finished
            rr_start_blockComponents = rr_start_block.components
            for thisComponent in rr_start_block.components:
                thisComponent.tStart = None
                thisComponent.tStop = None
                thisComponent.tStartRefresh = None
                thisComponent.tStopRefresh = None
            ...
            # --- Run Routine "rr_start_block" ---
            thisExp.currentRoutine = rr_start_block
            rr_start_block.forceEnded = routineForceEnded = not continueRoutine
            while continueRoutine:
                # if trial has changed, end Routine now
                if hasattr(thisYn_block, 'status') and thisYn_block.status == STOPPING:
                    continueRoutine = False
                # get current time
                t = routineTimer.getTime()
                tThisFlip = win.getFutureFlipTime(clock=routineTimer)
                ...
                frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
                # update/draw components on each frame
                # Run 'Each Frame' code from start_block_code
                # Measure time
                remaining = max(0, BLOCK_BREAK_DURATION - t)
                if remaining <= 0:
                    continueRoutine = False
                
                # Update button label
                start_block_label = (
                    f'BLOCK STARTEN IN {int(remaining)}s\n'
                    + f'\n'
                    + f'oder hier Klicken'
                )
                ...
                if start_block_text.status == STARTED:
                    # update params
                    pass
                # *start_block_button* updates
                
                # if start_block_button is starting this frame...
                if start_block_button.status == NOT_STARTED and tThisFlip >= 0-frameTolerance:
                    # keep track of start time/frame for later
                    start_block_button.frameNStart = frameN  # exact frame index
                    start_block_button.tStart = t  # local t and not account for scr refresh
                    start_block_button.tStartRefresh = tThisFlipGlobal  # on global time
                    win.timeOnFlip(start_block_button, 'tStartRefresh')  # time at next scr refresh
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'start_block_button.started')
                    # update status
                    start_block_button.status = STARTED
                    win.callOnFlip(start_block_button.buttonClock.reset)
                    start_block_button.setAutoDraw(True)
                
                # if start_block_button is active this frame...
                if start_block_button.status == STARTED:
                    # update params
                    start_block_button.setText(start_block_label, log=False)
                    # check whether start_block_button has been pressed
                    if start_block_button.isClicked:
                        if not start_block_button.wasClicked:
                            # if this is a new click, store time of first click and clicked until
                            start_block_button.timesOn.append(start_block_button.buttonClock.getTime())
                            start_block_button.timesOff.append(start_block_button.buttonClock.getTime())
                        elif len(start_block_button.timesOff):
                            # if click is continuing from last frame, update time of clicked until
                            start_block_button.timesOff[-1] = start_block_button.buttonClock.getTime()
                        if not start_block_button.wasClicked:
                            # end routine when start_block_button is clicked
                            continueRoutine = False
                        if not start_block_button.wasClicked:
                            # run callback code when start_block_button is clicked
                            pass
                # take note of whether start_block_button was clicked, so that next frame we know if clicks are new
                start_block_button.wasClicked = start_block_button.isClicked and start_block_button.status == STARTED
                ...
                # has a Component requested the Routine to end?
                if not continueRoutine:
                    rr_start_block.forceEnded = routineForceEnded = True
                # has the Routine been forcibly ended?
                if rr_start_block.forceEnded or routineForceEnded:
                    break
                # has every Component finished?
                continueRoutine = False
                for thisComponent in rr_start_block.components:
            ...
            # --- Ending Routine "rr_start_block" ---
            for thisComponent in rr_start_block.components:
                if hasattr(thisComponent, "setAutoDraw"):
                    thisComponent.setAutoDraw(False)
            # store stop times for rr_start_block
            rr_start_block.tStop = globalClock.getTime(format='float')
            rr_start_block.tStopRefresh = tThisFlipGlobal
            thisExp.addData('rr_start_block.stopped', rr_start_block.tStop)
            yn_block.addData('start_block_button.numClicks', start_block_button.numClicks)
            if start_block_button.numClicks:
               yn_block.addData('start_block_button.timesOn', start_block_button.timesOn)
               yn_block.addData('start_block_button.timesOff', start_block_button.timesOff)
            else:
               yn_block.addData('start_block_button.timesOn', "")
               yn_block.addData('start_block_button.timesOff', "")
            # the Routine "rr_start_block" was not non-slip safe, so reset the non-slip timer
            routineTimer.reset()
            ...
            for thisOo_level in oo_level:
                ...
                force_nrmse_buffer = []
                target_pmvc    = cur_physical_demand_level * mvc
                lower_threshold = max(target_pmvc - TOLERANCE, 0)
                upper_threshold = target_pmvc + TOLERANCE
                ...
                # ===== DISPLAY SETUP =====
                target_display_text = "Waiting for force signal..."
                ...
                # ===== INITIALIZE FORCE METRICS =====
                FORCE_TOLERANCE_BUFFER.clear()
                force_nrmse_buffer = []
                target_lights_led_off_sent = False
                ...
                while continueRoutine and routineTimer.getTime() < 3.0:
                    ...
                    if target_force.whiteForce is not None:
                        cur_force = max(target_force.whiteForce, 0)
                        
                        # Populate the force buffer for smooth feedback
                        FORCE_TOLERANCE_BUFFER.append(cur_force)
                        smooth_force = statistics.median(FORCE_TOLERANCE_BUFFER)
                        
                        # Collect force value for deviation
                        force_nrmse_buffer.append(smooth_force)
                        nrmse = force_nrmse(force_nrmse_buffer, target_pmvc)
                        sbias = force_bias(force_nrmse_buffer, target_pmvc)
                        ptwtr = force_pct_in_range(force_nrmse_buffer, target_pmvc)
                        
                        if len(FORCE_TOLERANCE_BUFFER) >= FORCE_BUFFER_LENGTH:
                    ...
                    target_display_text = (
                        ...
                        f"----------------------------------------\n"
                        f"MVC:               {mvc:03.0f} N\n"
                        f"Target:            {lower_threshold:03.0f} N | {target_pmvc:03.0f} N | {upper_threshold:03.0f} N\n"
                        f"Force:                     {smooth_force:03.0f} N\n"
                        f"Cur. Deviation:            {deviation_symbol}\n"
                        f"NRMSE:             {nrmse:6.0f}\n"
                        f"Bias:              {sbias:+6.0f}\n"
                        f"pTWTR:             {ptwtr:6.0f}\n"
                        f"========================================\n"
                    ...
                    if target_lights.status == STARTED:
                        ...
                        if tThisFlipGlobal > target_lights.tStartRefresh + 3.0-frameTolerance:
                            ...
                            if True:
                               ...
                               target_lights_led_off_sent = True
                ...
                force_nrmse_buffer = []
                if True and not target_lights_led_off_sent:
                    target_lights.turnOffLights("inner")
                ...
                if True:
                    ...
                    _trial_index = _loop.thisN if _loop is not None and hasattr(_loop, 'thisN') else -1
                    _trial_name = _loop.name if _loop is not None and hasattr(_loop, 'name') else ''
                    _identifier = str("target presentation") if "target presentation" is not None else ''
                    _records = target_force.forceRows if hasattr(target_force, 'forceRows') else []
                    for _i, _record in enumerate(_records):
                        target_force_raw_buffer.append({
                            'participant': expInfo.get('participant', ''),
                            'session': expInfo.get('session', ''),
                            'routine': 'rr_target',
                            'component': 'target_force',
                            'trial_index': _trial_index,
                            'trial_name': _trial_name,
                            'identifier': _identifier,
                            'sample_index': _i,
                ...
                for thisOo_palette in oo_palette:
                    ...
                    rr_trial = data.Routine(
                        name='rr_trial',
                        components=[trial_lights, trial_reed, trial_force, trial_display],
                    )
                    ...
                    trial_lights_led_off_sent = False
                    ...
                    while continueRoutine:
                        ...
                        if trial_force.whiteForce is not None:
                            cur_force = max(trial_force.whiteForce, 0)
                        ...
                        if trial_lights.status == STARTED:
                            ...
                            if tThisFlipGlobal > trial_lights.tStartRefresh + PALETTE_DURATION_SEC-frameTolerance:
                                ...
                                if True:
                                   ...
                                   trial_lights_led_off_sent = True
                    ...
                    if False and not trial_lights_led_off_sent:
                        trial_lights.turnOffLights("inner")
                    if trial_reed._reed_measuring:   trial_reed.stopReedMeasurement()
                    ...
                    if True:
                        ...
                        _trial_index = _loop.thisN if _loop is not None and hasattr(_loop, 'thisN') else -1
                        _trial_name = _loop.name if _loop is not None and hasattr(_loop, 'name') else ''
                        _identifier = str("trial") if "trial" is not None else ''
                        _records = trial_force.forceRows if hasattr(trial_force, 'forceRows') else []
                        for _i, _record in enumerate(_records):
                            trial_force_raw_buffer.append({
                                'participant': expInfo.get('participant', ''),
                                'session': expInfo.get('session', ''),
                                'routine': 'rr_trial',
                                'component': 'trial_force',
                                'trial_index': _trial_index,
                                'trial_name': _trial_name,
                                'identifier': _identifier,
                                'sample_index': _i,
                ...
                if thisSession is not None:
                    ...
                    thisSession.sendExperimentData()
                # get names of stimulus parameters
                if oo_palette.trialList in ([], [None], None):
                    params = []
                else:
                    params = oo_palette.trialList[0].keys()
                # save data for this loop
                oo_palette.saveAsExcel(filename + '.xlsx', sheetName='oo_palette',
                    stimOut=params,
                    dataOut=['n','all_mean','all_std', 'all_raw'])
                ...
                feedback_display_text = (
                    f"==== PERFORMANCE ====\n"
                    f"d-prime:       {d_prime:+5.2f}\n"
                    f"Hits:          {hits:5}\n"
                    f"Misses:        {misses:5}\n"
                    f"False Alarms:  {false_alarms:5}\n"
                    f"Correct Rej.:  {correct_rejections:5}\n"
                    f"---------------------\n"
                    f"NRMSE:         {nrmse:5.0f}\n"
                    f"Bias:          {sbias:+5.0f}\n"
                    f"pTWR:          {ptwtr:5.0f}\n"
                ...
                while continueRoutine:
                    ...
                    if rating_continue_button.status == NOT_STARTED and is_ratings_complete or speedrun:
                        # keep track of start time/frame for later
            ...
            if thisSession is not None:
                ...
                thisSession.sendExperimentData()
            # get names of stimulus parameters
            if oo_level.trialList in ([], [None], None):
                params = []
            else:
                params = oo_level.trialList[0].keys()
            # save data for this loop
            oo_level.saveAsExcel(filename + '.xlsx', sheetName='oo_level',
                stimOut=params,
                dataOut=['n','all_mean','all_std', 'all_raw'])
            # mark thisYn_block as finished
            if hasattr(thisYn_block, 'status'):
                thisYn_block.status = FINISHED
            # if awaiting a pause, pause now
            if yn_block.status == PAUSED:
                thisExp.status = PAUSED
                pauseExperiment(
                    thisExp=thisExp, 
                    win=win, 
        ...
        # completed is_block repeats of 'yn_block'
        yn_block.status = FINISHED
        
        if thisSession is not None:
            # if running in a Session with a Liaison client, send data up to now
            thisSession.sendExperimentData()
        # get names of stimulus parameters
        if yn_block.trialList in ([], [None], None):
            params = []
        else:
            params = yn_block.trialList[0].keys()
        # save data for this loop
        yn_block.saveAsExcel(filename + '.xlsx', sheetName='yn_block',
            stimOut=params,
            dataOut=['n','all_mean','all_std', 'all_raw'])
    ...
    # completed len(blocks) + 1 repeats of 'oo_block'
    ...
    if thisSession is not None:
        ...
        thisSession.sendExperimentData()
    # get names of stimulus parameters
    if oo_block.trialList in ([], [None], None):
        params = []
    else:
        params = oo_block.trialList[0].keys()
    # save data for this loop
    oo_block.saveAsExcel(filename + '.xlsx', sheetName='oo_block',
        stimOut=params,
        dataOut=['n','all_mean','all_std', 'all_raw'])
    
    # --- Prepare to start Routine "rr_finish" ---
    # create an object to store info about Routine rr_finish
    rr_finish = data.Routine(
        name='rr_finish',
        components=[finish_display],
    )
    rr_finish.status = NOT_STARTED
    continueRoutine = True
    # update component parameters for each repeat
    # store start times for rr_finish
    rr_finish.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
    rr_finish.tStart = globalClock.getTime(format='float')
    rr_finish.status = STARTED
    thisExp.addData('rr_finish.started', rr_finish.tStart)
    rr_finish.maxDuration = None
    # keep track of which components have finished
    rr_finishComponents = rr_finish.components
    for thisComponent in rr_finish.components:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    # --- Run Routine "rr_finish" ---
    thisExp.currentRoutine = rr_finish
    rr_finish.forceEnded = routineForceEnded = not continueRoutine
    while continueRoutine and routineTimer.getTime() < 10.0:
        ...
        # *finish_display* updates
        
        # if finish_display is starting this frame...
        if finish_display.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            finish_display.frameNStart = frameN  # exact frame index
            finish_display.tStart = t  # local t and not account for scr refresh
            finish_display.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(finish_display, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'finish_display.started')
            # update status
            finish_display.status = STARTED
            finish_display.setAutoDraw(True)
        
        # if finish_display is active this frame...
        if finish_display.status == STARTED:
            # update params
            pass
        
        # if finish_display is stopping this frame...
        if finish_display.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > finish_display.tStartRefresh + 10.0-frameTolerance:
                # keep track of stop time/frame for later
                finish_display.tStop = t  # not accounting for scr refresh
                finish_display.tStopRefresh = tThisFlipGlobal  # on global time
                finish_display.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'finish_display.stopped')
                # update status
                finish_display.status = FINISHED
                finish_display.setAutoDraw(False)
        ...
        # has a Component requested the Routine to end?
        if not continueRoutine:
            rr_finish.forceEnded = routineForceEnded = True
        # has the Routine been forcibly ended?
        if rr_finish.forceEnded or routineForceEnded:
            break
        # has every Component finished?
        continueRoutine = False
        for thisComponent in rr_finish.components:
    ...
    # --- Ending Routine "rr_finish" ---
    for thisComponent in rr_finish.components:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    # store stop times for rr_finish
    rr_finish.tStop = globalClock.getTime(format='float')
    rr_finish.tStopRefresh = tThisFlipGlobal
    thisExp.addData('rr_finish.stopped', rr_finish.tStop)
    # using non-slip timing so subtract the expected duration of this Routine (unless ended on request)
    if rr_finish.maxDurationReached:
        routineTimer.addTime(-rr_finish.maxDuration)
    elif rr_finish.forceEnded:
        routineTimer.reset()
    else:
        routineTimer.addTime(-10.000000)
    thisExp.nextEntry()
    if True and maxforce_force_raw_buffer:
        ...
        with open(_raw_path, 'w', encoding='utf-8') as _f:
            ...
            for _row in maxforce_force_raw_buffer:
    ...
    if True and target_force_raw_buffer:
        _raw_path = thisExp.dataFileName + '_force_long.tsv'
        with open(_raw_path, 'w', encoding='utf-8') as _f:
            _f.write('participant\tsession\troutine\tcomponent\ttrial_index\ttrial_name\tidentifier\tsample_index\twhite_time\tblue_time\ttime\twhite_force\tblue_force\twhite_force_raw_counts\tblue_force_raw_counts\n')
            for _row in target_force_raw_buffer:
                _f.write('\t'.join(str(_row[_k]) for _k in ['participant','session','routine','component','trial_index','trial_name','identifier','sample_index','white_time','blue_time','time','white_force','blue_force','white_force_raw_counts','blue_force_raw_counts']) + '\n')
    if True and trial_force_raw_buffer:
        _raw_path = thisExp.dataFileName + '_force_long.tsv'
        with open(_raw_path, 'w', encoding='utf-8') as _f:
            _f.write('participant\tsession\troutine\tcomponent\ttrial_index\ttrial_name\tidentifier\tsample_index\twhite_time\tblue_time\ttime\twhite_force\tblue_force\twhite_force_raw_counts\tblue_force_raw_counts\n')
            for _row in trial_force_raw_buffer:
                _f.write('\t'.join(str(_row[_k]) for _k in ['participant','session','routine','component','trial_index','trial_name','identifier','sample_index','white_time','blue_time','time','white_force','blue_force','white_force_raw_counts','blue_force_raw_counts']) + '\n')
